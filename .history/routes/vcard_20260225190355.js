...
router.get('/:id', async (req, res) => {
  try {
    ...
    // formatted linkedin text for display
    let linkedinLabel = '';
    if (data.linkedin) {
      linkedinLabel = data.linkedin
        .replace(/^https?:\/\/(www\.)?linkedin\.com\/in\//i, '') // strip prefix
        .replace(/\/$/, '') // remove trailing slash
        // remove trailing non-letter characters (numbers, symbols)
        .replace(/[^a-zA-Z\-\s]+$/, '')
    ...
    <div class="vcard-info">
      ...
      <div class="info-item">
        ...
        <div class="info-content">
          ...
          <div class="info-value"><a href="${data.linkedin}" target="_blank" rel="noopener noreferrer">${linkedinLabel}</a></div>
        </div>
      </div>
      ` : ''}
