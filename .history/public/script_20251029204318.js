document.addEventListener('DOMContentLoaded', function() {
  ...
  function displayCreations(creations) {
    ...
    if (creations.length === 0) {
      creationsList.innerHTML = '<p>Você ainda não criou nenhum QR Code.</p>';
      return;
    }
    ...
    creations.forEach(creation => {
      ...
      creationDiv.innerHTML = `
        <div class="creation-info">
          ...
          <p>Criado em: ${new Date(creation.created_at).toLocaleString()}</p>
        </div>
    ...
    // Adiciona event listeners para botões de download
    document.querySelectorAll('.download-btn').forEach(btn => {
      btn.addEventListener('click', (e) => downloadCreation(e.target.dataset.id));
    });
  }
  ...
  function showCreationsMessage(text, type) {
    const messageDiv = document.getElementById('creations-message');
